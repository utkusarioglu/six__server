...
import { UserUpPl } from './user.model.types';
...
`, () => {
  ...
  it('Creates the table with the expected specs', async (done) => {
    ...
    tracker.on('query', (query) => {
      if (query.sql.toUpperCase().startsWith('CREATE TABLE')) {
        ...
        const columns: UserUpPl['_db']['OutT'] = {
          id: '',
          username: '',
          password: '',
          email: '',
  ...
  it('Can build functioning user select query', async (done) => {
    let queryHit = 0;
    const emailColumn: keyof UserUpPl['_insert']['OutT'] = 'email';
    const randomEmail: UserUpPl['_insert']['OutT']['email'] = chance.word();
  ...
  it('Returns user if only one user is found', async (done) => {
    ...
    const randomUser: UserUpPl['_db']['OutT'] = {
      id: chance.guid(),
  ...
  it('Returns false if more than one user is found', async (done) => {
    ...
    const randomUsers: UserUpPl['_insert']['OutT'][] = Array(3).fill({
      id: chance.guid(),
      username: chance.word(),
      password: chance.word(),
      email: chance.email(),
